...
.result-box{background:rgba(0,0,0,.5);border-radius:12px;padding:12px;margin-top:10px;max-height:180px;overflow-y:auto;text-align:left}
...
<div class="stats"><div class="stat-box"><div class="num green">{tv}</div><div class="label">TỔNG</div></div>
<div class="stat-box"><div class="num pink">{uv}</div><div class="label">ĐÃ DÙNG</div></div>
<div class="stat-box"><div class="num blue">{tv-uv}</div><div class="label">CÒN</div></div></div>
...
function createKeys(){{
    ...
    .then(async r => {{
        ...
        if (!r.ok) {{
            throw new Error(d.error || "API lỗi: " + r.status);
        }}
    ...
    .then(d => {{
        ...
        if (!d.keys || !Array.isArray(d.keys)) {{
            throw new Error("API không trả về danh sách key");
        }}

        b.style.display = "block";

        b.innerHTML = d.keys.map(k =>
            '<div class="result-item">' +
            '<span style="color:#00ff88;">' + k.key + '</span>' +
            '<span style="font-size:10px;">' + (k.expiry || "Chưa kích hoạt") + '</span>' +
            '<button class="btn btn-copy" style="padding:4px 8px;font-size:10px;width:auto;" onclick="copyKey(\\'' + k.key + '\\')">COPY</button>' +
            '</div>'
        ).join("");

        showToast("✅ Đã tạo " + d.keys.length + " key!");
