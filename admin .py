...
def api_verify():
    ...
    if datetime.strptime(expiry,"%Y-%m-%d %H:%M:%S") < datetime.now(): conn.close(); return jsonify({"valid":False,"reason":"Key hết hạn!"})
    if used and dev and dev != device_id: conn.close(); return jsonify({"valid":False,"reason":"Key đã dùng thiết bị khác!"})
    if not used: c.execute("UPDATE vip_keys SET device_id=?, is_used=1 WHERE key_code=?",(device_id,key_code)); conn.commit()
    conn.close()
